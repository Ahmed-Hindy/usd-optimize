...
def __resolve_valid_operations(operations: list, supports_analysis: bool, result: dict):
    ...
    operation_names = UsdOptimizeCore.getInstance().getOperations()
    ...
    for operation in operations:
        ...
        if supports_analysis and not UsdOptimizeCore.getInstance().getOperationSupportsAnalysis(operation.name):
...
def analyze(stage: Usd.Stage, operations: list, context: ExecutionContext = None):
    ...
    # create a new Usd Optimize ExecutionContext in analysis mode
    if context is None:
    ...
    for operation in valid_operations:
        operation_result = UsdOptimizeCore.getInstance().executeOperation(operation.name, context, operation.args)
        analysis_data = {}
...
def optimize(stage: Usd.Stage, operations: list, context: ExecutionContext = None):
    """
    Runs Usd Optimize for the given operations on the stage, and uses the analysis result to configure the operations
    if available.
    ...
    # create a new Usd Optimize ExecutionContext if needed
    if context is None:
        context = ExecutionContext()
    context.usdStageId = UsdUtils.StageCache().Get().Insert(stage).ToLongInt()
