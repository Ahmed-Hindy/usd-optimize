...
// Geometry for a single unit quad in the z=0 plane with its lower corner at (minX, minY), used by the
// coincident-boundary clustering tests below. The returned arrays must outlive any BoundaryMeshData that references
// them.
struct TestQuad
{
    VtVec3fArray points;
...
static TestQuad makeQuad(float minX, float minY = 0.0f)
{
    TestQuad quad;
    quad.points = { GfVec3f(minX, minY, 0.0f),
                    GfVec3f(minX + 1.0f, minY, 0.0f),
                    GfVec3f(minX + 1.0f, minY + 1.0f, 0.0f),
                    GfVec3f(minX, minY + 1.0f, 0.0f) };
    quad.faceVertexCounts = { 4 };
    quad.faceVertexIndices = { 0, 1, 2, 3 };
    return quad;
}

static BoundaryMeshData makeData(const TestQuad& quad, const GfMatrix4d& localToWorld = GfMatrix4d(1.0))
{
    BoundaryMeshData data;
    data.points = &quad.points;
    data.faceVertexCounts = &quad.faceVertexCounts;
    data.faceVertexIndices = &quad.faceVertexIndices;
    data.localToWorld = localToWorld;
    return data;
}


TEST_CASE("clusterByCoincidentBoundary groups meshes sharing a seam")
{
    // A-B-C abut edge-to-edge in a row (each shares two boundary vertices with its neighbor); D is detached.
    TestQuad a = makeQuad(0.0f);
    TestQuad b = makeQuad(1.0f);
    TestQuad c = makeQuad(2.0f);
    TestQuad d = makeQuad(10.0f);

    std::vector<BoundaryMeshData> meshes = { makeData(a), makeData(b), makeData(c), makeData(d) };
    std::vector<int> clusters(meshes.size(), INVALID_CLUSTER);

    clusterByCoincidentBoundary(meshes, 1e-5, /*minSharedVertices*/ 2, clusters);

    // A, B and C share seams transitively, so they land in one cluster.
    CHECK_NE(clusters[0], INVALID_CLUSTER);
    CHECK_EQ(clusters[0], clusters[1]);
    CHECK_EQ(clusters[1], clusters[2]);

    // D shares no boundary with anything and must stay unclustered.
    CHECK_EQ(clusters[3], INVALID_CLUSTER);
}


TEST_CASE("clusterByCoincidentBoundary respects the minimum shared vertex count")
{
    // Quad A and a quad touching only at the single corner vertex (1, 1).
    TestQuad a = makeQuad(0.0f, 0.0f);
    TestQuad corner = makeQuad(1.0f, 1.0f);

    SUBCASE("a single shared vertex is not enough by default")
    {
        std::vector<BoundaryMeshData> meshes = { makeData(a), makeData(corner) };
        std::vector<int> clusters(meshes.size(), INVALID_CLUSTER);

        clusterByCoincidentBoundary(meshes, 1e-5, /*minSharedVertices*/ 2, clusters);

        CHECK_EQ(clusters[0], INVALID_CLUSTER);
        CHECK_EQ(clusters[1], INVALID_CLUSTER);
    }

    SUBCASE("a single shared vertex connects when the minimum is 1")
    {
        std::vector<BoundaryMeshData> meshes = { makeData(a), makeData(corner) };
        std::vector<int> clusters(meshes.size(), INVALID_CLUSTER);

        clusterByCoincidentBoundary(meshes, 1e-5, /*minSharedVertices*/ 1, clusters);
...
TEST_CASE("clusterByCoincidentBoundary compares in world space")
{
    // Two quads with identical local geometry; only the world transform makes their boundaries coincide.
    TestQuad local = makeQuad(0.0f);

    GfMatrix4d identity(1.0);
    GfMatrix4d shifted(1.0);
    shifted.SetTranslate(GfVec3d(1.0, 0.0, 0.0)); // places the second quad's left edge on the first's right edge

    SUBCASE("coincident after transform -> merged")
    {
        std::vector<BoundaryMeshData> meshes = { makeData(local, identity), makeData(local, shifted) };
        std::vector<int> clusters(meshes.size(), INVALID_CLUSTER);
    ...
    SUBCASE("non-positive tolerance merges nothing")
    {
        std::vector<BoundaryMeshData> meshes = { makeData(local, identity), makeData(local, shifted) };
        std::vector<int> clusters(meshes.size(), INVALID_CLUSTER);

        clusterByCoincidentBoundary(meshes, 0.0, /*minSharedVertices*/ 2, clusters);
