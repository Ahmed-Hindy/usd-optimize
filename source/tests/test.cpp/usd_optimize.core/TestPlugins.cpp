...
{
    return UsdOptimizeCore::getInstance().getOperation(kTestPluginName);
}


// Register plugin with Usd Optimize
USD_OPTIMIZE_PLUGIN_INIT(TestOperation);
...
TEST_CASE("Test Plugins")
{
    ...
    SUBCASE("Test plugins loaded")
    {
        ...
        UsdOptimizePluginVersion version = plugin->getVersion();
    ...
    SUBCASE("Test unloading and reloading plugin")
    {
        ...
        core.registerOperation(&usdOptimizeOperationCreate<TestOperation>, &usdOptimizeOperationDelete<TestOperation>);
    ...
    SUBCASE("Test authors and versions")
    {
        ...
        for (const auto& name : operations)
        {
            ...
            UsdOptimizePluginVersion version = op->getVersion();
