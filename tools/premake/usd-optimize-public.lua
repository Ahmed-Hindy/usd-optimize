...
-- Convenience function to build against the standalone core usd optimize library
function m.use_usd_optimize_core()
...
-- @param library_name: The base name of the library to build (without "lib" prefix, "usd_optimize" prefix, or file extension)
...
function m.shared_library(options)
    ...
    targetname("usd_optimize."..library_name)
...
function m.operation_plugin(sources)
    ...
    defines {"USD_OPTIMIZE_PLUGIN_AUTHOR="..plugin_author}
    ...
    filter { "system:windows" }
    files("%{root}/_build/usd-optimize.rc")
    filter {}
    ...
    -- Link against the usd optimize core
    m.use_usd_optimize_core()
