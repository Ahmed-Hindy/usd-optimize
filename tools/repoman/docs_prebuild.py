...
_VALIDATION_PACKAGE = "usd-validation-nvidia>=1.19.3"
_VALIDATION_IMPORT = "usd_validation_nvidia"
...
def _ensure_validation_package(python: str, env: dict) -> None:
    """Install ``usd-validation-nvidia`` into the bundled interpreter if missing.
    ...
    by the build, so pip-install it into the bundled interpreter (idempotent: skip
    if it already imports). Mirrors the check in ``tools/perf_validators/run.sh``.
    """
    already_installed = subprocess.run(
        [python, "-c", f"import {_VALIDATION_IMPORT}"],
        env=env,
        capture_output=True,
    ).returncode == 0
    ...
    print(f"[docs_prebuild] installing {_VALIDATION_PACKAGE} into bundled interpreter")
