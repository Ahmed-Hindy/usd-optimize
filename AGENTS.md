...
Many rules attach a fix suggestion, so `usd-validation-nvidia`'s `IssueFixer` can apply them automatically — this is what `tools/validators/run.sh <asset> --fix` drives. `--fix` applies fixes **in place** to the asset you pass, so copy the asset first if you want to preserve the original. The authoritative rule list and per-rule descriptions are in `docs/performance-validators.rst` (generated from the rule classes).
