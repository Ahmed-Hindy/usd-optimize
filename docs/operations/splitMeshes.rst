...
When the ``Spatial Clustering Mode`` is enabled meshes will be split and merged spatially by user specified values. This also greatly improves processing performance when running in this mode to split and merge meshes in a single process versus running each operation separately.
